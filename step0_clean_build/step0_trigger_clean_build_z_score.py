...
def main() -> int:
    ...
    out_all_path = out_dir / "data_trigger_clean_z.csv"
    out_main_path = out_dir / "data_trigger_mainmodel_z.csv"

    _log("Step0 | Align + Derive (trigger + weather)")
    _log(f"[ROOT ] {root}")
    _log(f"[IN   ] {trigger_path}")
    _log(f"[IN   ] {weather_path}")
    _log(f"[OUT  ] {out_all_path}")
    _log(f"[OUT  ] {out_main_path}")
    _log(f"[CFG  ] RESAMPLE_RULE={RESAMPLE_RULE}, AGG_METHOD={AGG_METHOD}, ZSCORE_WEATHER={ZSCORE_WEATHER}")
    ...
    if len(df) == 0:
        raise RuntimeError(
            "Merged dataframe is empty. Check time overlap between trigger and weather."
        )
    ...
    if not ZSCORE_WEATHER:
        weather_cols = set(weather_rs.columns)
        exclude = [c for c in numeric_cols if (c in weather_cols) or c.endswith("__weather")]
        z_cols = [c for c in numeric_cols if c not in exclude]
    else:
        z_cols = numeric_cols

    df_z = _zscore(df, z_cols)

    df_out = df_z.reset_index().rename(columns={df_z.index.name: "time"})
    df_out.to_csv(out_all_path, index=False, encoding=CSV_ENCODING)
    _log(f"[DONE] Saved (all): {out_all_path}")
    _log(f"[INFO] Output(all) shape: {df_out.shape[0]:,} rows × {df_out.shape[1]:,} cols")
    ...
    _log(f"[DONE] Saved (main-model): {out_main_path}")
    _log(f"[INFO] Output(main) shape: {df_main.shape[0]:,} rows × {df_main.shape[1]:,} cols")
